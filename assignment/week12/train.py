...
if __name__ == '__main__':
    ...
    model.compile(loss='mean_squared_error', optimizer=ko.SGD(lr=0.1, decay=0.001, momentum=0.9))

    
    #ku.plot_model(model, 'model.png') # 모델 구조 그리기
    ...
    print "1 Epoch Batch Size = %d\n" % (_BATCH_SIZE)
    fdTrainLog.write("1 Epoch Batch Size = %d\n\n" % (_BATCH_SIZE))
    
    # 학습 시작 (2000번 이상 학습하거나 Test set에 대한 에러율이 8% 미만으로 나오면 중단)
    while trainCnt < 2000:
        # 1 Epoch 학습 진행 (Batch Size 만큼의 데이터로 1회)
        epoch = trainCnt % iterationNum
        trLoss = model.train_on_batch(trDataList[trIndexList[epoch]], trLabelList[trIndexList[epoch]])
        
        # Test Set 으로 에러율 검사
        result = model.predict(trDataList[tsIndexList], batch_size=_BATCH_SIZE)
